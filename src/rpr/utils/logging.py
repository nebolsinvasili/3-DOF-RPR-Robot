...
def data_B(data, rpr, ref=None):
    if ref is not None:
        for i, value in enumerate(Platform.distance_attact_joints(ref.platform.coord, ref.platform.joints)):
            data.update({f'Bref{i+1}_dj': value})

        for i, value in enumerate(Platform.angle_attact_joints(ref.platform.coord, ref.platform.joints)):
    ...
    for i, value in enumerate(Platform.distance_attact_joints(rpr.platform.coord, rpr.platform.joints)):
        data.update({f'B{i+1}_dj': value})

    for i, value in enumerate(Platform.angle_attact_joints(rpr.platform.coord, rpr.platform.joints)):
...
def data_leg(data, rpr, ref=None):
    if ref is not None:
        for i, distance in enumerate(Leg.get_distance_legs(ground_joints=ref.ground.joints, platform_joints=ref.platform.joints)):
            data.update({f'Lref{i+1}_d': distance})

        for i, angle in enumerate(Leg.get_angle_leg(ground_joints=ref.ground.joints, platform_joints=ref.platform.joints)):
            data.update({f'Lref{i+1}_a': angle})
    
    for i, distance in enumerate(Leg.get_distance_legs(ground_joints=rpr.ground.joints, platform_joints=rpr.platform.joints)):
        data.update({f'L{i+1}d': distance})

    for i, angle in enumerate(Leg.get_angle_leg(ground_joints=rpr.ground.joints, platform_joints=rpr.platform.joints)):
